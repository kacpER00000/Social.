...
const MoreButton = ({show, onShowClicked}: MoreButtonProps) => {
    return(
        <button className={`${show ? "bg-blue-600" : "bg-blue-500"} rounded-full px-6 py-2 text-sm font-bold text-white transition-colors duration-300 ease-in-out hover:bg-blue-600`} onClick={onShowClicked}><i className="icon-ellipsis"></i></button>
    );
}
