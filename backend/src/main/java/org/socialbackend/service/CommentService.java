...
public class CommentService {
     ...
     * @throws NoSuchElementException if either the target post or the logged-in
     *                                user does not exist.
     */
    @Transactional
     ...
     * @param commentRequest The request object containing the updated comment
     *                       content.
     ...
     * @throws NoSuchElementException if the comment does not exist.
     * @throws IllegalStateException  if the user lacks permissions (is neither the
     *                                comment owner nor the post owner).
     */
    @Transactional
    ...
    public CommentDTO findCommentByCommentId(Long commentId, Long userId) {
        return commentRepository.findById(commentId).map(c -> mapToDTO(c, userId))
                .orElseThrow(() -> new NoSuchElementException("Comment with this id don't exist"));
    }
    ...
    public Page<CommentDTO> findPostComments(Long postId, Pageable pageable, Long userId) {
        return commentRepository.findAllByPost_PostIdOrderByCreatedAtAsc(postId, pageable)
     ...
     * @param comment The comment to validate.
     * @param userId  The ID of the user.
     * @throws IllegalStateException if the user is neither the comment's author nor
     *                               the post's author.
     */
    ...
    private CommentDTO mapToDTO(Comment comment, Long userId) {
        ...
        return new CommentDTO(comment.getCommentId(), comment.getPost().getPostId(), ownerUserId, owner.getImgUrl(),
                author, comment.getContent(), comment.getCreatedAt(), canEdit, canDelete, isAuthorFollowed);
    }
